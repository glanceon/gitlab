...
export default async (pluginConfig, context) => {
  ...
  const { gitlabToken, gitlabUrl, repoUrl, gitlabApiUrl, assets, milestones, proxy } = resolveConfig(pluginConfig, context);
  const assetsList = [];
        ...
        if (_url) {
          ...
          if (target === "generic_package") {
            ...
            uploadEndpoint = urlJoin(
              gitlabApiUrl,
              `/projects/${repoUrl}/packages/generic/release/${encodedVersion}/${encodedLabel}?${
